...
' ------------------------------------------------------------------------
    ...
    s = mBasic.Align(SyncActionString(d_action), enAlignLeft, lMaxLenAction, sNbsp, " ")
    s = s & mBasic.Align(d_direction, enAlignLeft, lMaxLenDir, sNbsp, " ")
    s = s & mBasic.Align(SyncKindString(d_kind), enAlignLeft, lMaxLenKind) & " "
    v = Split(d_id, SYNC_ID_SEPARATOR)
    s = s & mBasic.Align(v(0), enAlignLeft, lMaxLenIdPart1) & "  "
    If UBound(v) >= 1 Then s = s & mBasic.Align(v(1), enAlignLeft, lMaxLenIdPart2) & "  "
...
' ------------------------------------------------------------------------------
    With LogServiced
        .ColsSpecs = "L10, L" & Serviced.MaxLenType & ", L90, L15, L150"
        .ColsHeader = "Due , Item, Item, Sync  , Comment"
        .ColsHeader = "Sync, Type, Id  , Result,        "
    End With
