...
' -----------------------------------------------------------------------------
    ...
    If mBasic.ArryIsAllocated(aLinks) Then
        For Each v In aLinks
            Services.LogItem = cls_wsh_target
            mSync.TargetWorkingCopy.BreakLink v, xlLinkTypeExcelLinks
            DoEvents
            LogServiced.ColsItems "obsolete", "Worksheet", SyncId(cls_wsh_target), "removed", "(Back)Link to '" & Split(v, "\")(UBound(Split(v, "\"))) & "' cleared"
        Next v
    End If
        ...
        If InStr(nme.RefersTo, mSync.Source.Name) <> 0 Then
            ...
            If InStr(nme.RefersTo, "[" & mSync.Source.Name & "]") <> 0 Then
                ...
                Else
                    ...
                    LogServiced.ColsItems "invalid", "Worksheet", Services.ServicedItemName, "changed", "Back-Reference '" & sRefersTo & "' changed to '" & nme.RefersTo & "'"
                End If
            End If
        End If
    Next nme
        ...
        For Each shp In wsh.Shapes
            ...
            If Err.Number = 0 Then ' shape has an OnAction property
                If InStr(sOnAction, mSync.Source.Name) <> 0 Then
                    ...
                    LogServiced.ColsItems "invalid", "Worksheet", Services.ServicedItemName, "changed", "Back-Link to source '" & sOnAction & "' changed to '" & shp.OnAction & "'"
                End If
            End If
        Next shp
    Next wsh
