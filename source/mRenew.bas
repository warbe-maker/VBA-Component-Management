...
' ------------------------------------------------------------------------------
    ...
    With bi_wbk_serviced.VBProject
        ...
        If mComp.Exists(xst_wbk:=bi_wbk_serviced, xst_vbc_name:=bi_vbc_name) Then
            ...
            '~~ should prevent this.
            OutCommentCodeInRenamedComponent bi_wbk_serviced, sTempName ' this had made it much less "reliablele"
        ...
        With Comp
            ...
            .CompName = bi_vbc_name
        End With
        .VBComponents(bi_vbc_name).Export Comp.ExpFileFullName
...
Public Sub Run(ByRef r_wbk_serviced As Workbook, _
               ByVal r_vbc_name As String, _
               ByVal r_exp_file As String, _
      Optional ByVal r_hosted As String = vbNullString)
' ------------------------------------------------------------------------------
' Renews a Workbook's (r_wbk_serviced) component (r_vbc_name) by re-importing a
...
'
' Preconditions (not evaluated by the service):
' - r_wbk_serviced is an open Workbook
' - r_vbc_name is a components in its VB-Prtoject
...
' ------------------------------------------------------------------------------
    ...
    If Not r_wbk_serviced.Saved Then
        Step = Step + 1
        MonitorStep.Text = Step & ". Save serviced Workbook '" & r_wbk_serviced.Name & "'"
        mMsg.Monitor mon_title:=MonitorTitle _
                   , mon_text:=MonitorStep _
                   , mon_steps_displayed:=MONITORED_STEPS _
                   , mon_width_min:=70 _
                   , mon_pos:=MONITOR_POS
        mService.SaveWbk r_wbk_serviced
    End If
    
    Set Comp = New clsComp
    With Comp
        Set .Wrkbk = r_wbk_serviced
        .CompName = r_vbc_name
        Log.ServicedItem = .VBComp
    End With
    EstablishExecTraceFile r_wbk_serviced
    ...
    With r_wbk_serviced.VBProject
        If mComp.Exists(xst_wbk:=r_wbk_serviced, xst_vbc_name:=r_vbc_name) Then
        
            '~~ Rename an already existing component
            sTempName = mComp.TempName(tn_wbk:=r_wbk_serviced, tn_vbc_name:=r_vbc_name)
            Step = Step + 1
            MonitorStep.Text = Step & ". Rename the component '" & r_vbc_name & "' to '" & sTempName & "'."
            mMsg.Monitor MonitorTitle, MonitorStep
            .VBComponents(r_vbc_name).Name = sTempName
            
            '~~ Outcomment the renamed component's code
            Step = Step + 1
            MonitorStep.Text = Step & ". Outcomment all code lines in the renamed component."
            mMsg.Monitor MonitorTitle, MonitorStep
            OutCommentCodeInRenamedComponent r_wbk_serviced, sTempName ' this had made it much less "reliablele"
            mBasic.TimedDoEvents ErrSrc(PROC)
        ...
        Log.Entry = "'" & r_vbc_name & "' (re-)imported from '" & r_exp_file & "'"
        ...
        Set Comp = New clsComp
        With Comp
            Set Comp.Wrkbk = r_wbk_serviced
            .CompName = r_vbc_name
        End With
        .VBComponents(r_vbc_name).Export Comp.ExpFileFullName
        ...
        MonitorFooter.Text = "Outdated component '" & r_vbc_name & "' successfully updated."
        mMsg.MonitorFooter MonitorTitle, MonitorFooter
        mCompMan.UpdateOutdatedCommonComponents uo_wbk_serviced:=r_wbk_serviced, uo_hosted:=r_hosted
...
Public Function Outdated(ByVal od_wbk_serviced As Workbook) As Dictionary
' ------------------------------------------------------------------------------
' Returns the serviced Workbook's (od_wbk_serviced) outdated Common Components as
' Dictionary.
' ------------------------------------------------------------------------------
    Const PROC = "Outdated"
    ...
    mBasic.BoP ErrSrc(PROC)
    Set dctAll = mService.AllComps(od_wbk_serviced)
    SaveWbk od_wbk_serviced

    With od_wbk_serviced.VBProject
