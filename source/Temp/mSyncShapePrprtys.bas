    ...
    If c_shp_source.Type = msoOLEControlObject Then
        ...
        If Not mSyncShapes.CorrespondingOOB(shpSource, wshTarget, oobTarget) Is Nothing Then
            For enProperty = mSyncShapePrprtys.enPropertiesOOBFirst To mSyncShapePrprtys.enPropertiesOOBLast
                ...
                mService.ServicedItem = PropertyServiced(shpSource, wshTarget)
...
' ------------------------------------------------------------------------------
    ...
    If SyncPropertyAsserted(sa_en_property) Then
        ...
        End If
        Log.Entry mService.ServicedItemType, mService.ServicedItemName, "Property " & mBasic.Align(PropertyName(sa_en_property), lPropertyMaxLen, AlignLeft, , ".") & ": changed " & PropertyChange(sa_target_value, sa_source_value)
    Else
        Log.Entry mService.ServicedItemType, mService.ServicedItemName, "Property " & mBasic.Align(PropertyName(sa_en_property), lPropertyMaxLen, AlignLeft, , ".") & ": assertion failed for change " & PropertyChange(sa_target_value, sa_source_value)
    End If
...
' ------------------------------------------------------------------------------
    ...
    If mSyncShapePrprtys.PropertyValue(sc_en_property, oobTarget) = mSyncShapePrprtys.PropertyValue(sc_en_property, oobSource) Then
        ...
        Else
            Debug.Print mBasic.Align(sc_property, sc_property_max_len, , , ".") & " not changed " & mSyncShapePrprtys.PropertyChange(vTarget, vSource)
            Stop
        End If
    End If
