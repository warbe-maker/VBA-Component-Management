...
Public Enum enCntrlProperties
    ...
    enName
    enAccelerator
    enAdjustmentsApplication
    enAdjustmentsCount
    enAdjustmentsCreator
    enAdjustmentsItem
    enAdjustmentsParentName
    enAlignment
    enAlternativeText
    enAltHTML
    enApplication
        ...
        For Each shp1 In .Shapes
            ...
            Select Case shp1.Type
                Case msoOLEControlObject
                    CntrlName = wsSyncTestB.OLEObjects(j).name
                Case msoFormControl
                    CntrlName = shp1.name
                Case Else
...
' -------------------------------------------------
    ...
    With ws
        For Each shp1 In .Shapes
            ...
            Select Case shp1.Type
                Case msoOLEControlObject
                    CntrlType = "ActiveX-" & TypeName(wsSyncTestB.OLEObjects(j).Object)
                Case msoFormControl
                    CntrlType = "FormControl-" & CntrlTypeString(shp.FormControlType)
                Case Else
...
Public Sub SheetObjects(ByVal wsh As Worksheet)
    ...
    For i = 1 To wsh.Shapes.Count
        '~~ Loop through all kinds of Form-Controls
        Set shp = wsh.Shapes(i)
        Select Case shp.Type
            Case msoFormControl
                Debug.Print "Shapes(" & i & ") " & TypeName(shp) & ":", "Name = " & shp.name & ",", "Type = " & CntrlTypeString(shp.FormControlType)
            Case msoOLEControlObject
                j = j + 1
                Set oob = wsSyncTestB.OLEObjects(j)
                Debug.Print "Shapes(" & i & ") OLEObjects(" & j & ")", "Name = " & oob.name & ",", "Type = " & TypeName(oob.Object)
            Case Else
                Debug.Print "Shapes(" & i & ")", TypeName(shp) & ":", "Name = " & shp.name & ",", "Type = " & ShapeTypeString(shp.Type)
        End Select
    Next i
